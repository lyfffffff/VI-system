...
系统 MUST 采用“语义变量层（默认 `--vi-*`）-> `--el-*`/`--wb-*` 映射层”的主题注入机制，组件 SHALL 不直接依赖品牌变量实现。

#### Scenario: 主题切换变量注入
- **WHEN** 用户从 `teal` 切换到 `blue`
- **THEN** 系统 MUST 先更新语义变量层
- **AND** 系统 MUST 同步更新 `--el-*` 与 `--wb-*` 映射变量
- **AND** Element Plus 与 Workbench 视觉 MUST 同步变化
...
系统 MUST 同时支持 CSS fallback 与 JS 运行时注入的暗黑模式机制，以降低主题切换闪烁风险。
