...
当前 `packages/vi` 的主题能力已经可用，但在“语义变量分层完整性、Element Plus 基础组件风格一致性、业务局部覆盖边界”上仍有优化空间。随着 V2 开发即将启动，现在需要先把这批优化沉淀为可执行规范，避免实现阶段再次分散。
...
- `theme-system`：增强主题系统在变量分层、组件覆盖策略与局部覆盖优先级方面的规范要求。
