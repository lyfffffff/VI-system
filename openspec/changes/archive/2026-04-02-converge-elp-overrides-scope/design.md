...
设计含义：
- `semantic` 层保留语义直写能力（渐变、暗黑补偿、语义对比度）。
- `mapping` 层默认仅做变量映射；直写具体值必须登记白名单。

### 2) Storybook 预览样式分层
- `preview.css` 收敛为“宿主基础层”（颜色模式、根节点高度、通用工具类）。
- 新增 `preview.stories.css` 承载“Story 演示层”（演示布局、回归故事壳层）。

设计含义：
- 预览宿主与组件主题职责分离，减少 Story 样式向公共主题层扩散。
- 后续可按复用度将 `preview.stories.css` 中可沉淀规则回迁至公共样式层。
