...
### 9. 文档收敛到单一总结入口
- 决策：将项目说明内容收敛到 `总结.md`，移除 `VI-System.md` 与 `docs/guides/*` 冗余文档，`README.md` 保持轻入口并指向总结文档。
- 理由：减少多文档并行维护导致的口径漂移，降低团队查阅与同步成本。
- 备选：保留多份说明文档并人工同步。
  - 未选原因：历史上已出现重复内容和维护负担，不利于持续迭代。

### 10. 移除自动列宽 composable，收敛库导出面
- 决策：删除 `use-auto-table-col-width` 及其导出，Story 示例改为静态列宽配置。
- 理由：该能力与主题系统主能力（主题状态、变量链路、主题组件）弱相关，保留在主包导出面会增加维护与理解成本。
- 备选：继续保留该 composable 作为工具能力。
  - 未选原因：本轮目标是主题能力收敛，工具类能力应避免混入主题主入口。

### 11. README 增加主题组件最小接入步骤
- 决策：在 `README.md` 明确“初始化主题 + 挂载 ThemeDrawer + 消费 useViTheme”三步最小接入流程。
- 理由：降低接入门槛，避免团队仅从代码推断 API 用法导致接入不一致。
- 备选：仅保留 API 列表，不提供完整步骤。
  - 未选原因：可读性与可执行性不足，不利于新成员快速落地。
...
6. 完成文档收敛：合并内容到 `总结.md`，移除冗余文档并在 OpenSpec 中记录增量约束。
7. 清理非主题核心导出并同步 Story 示例与 README 接入步骤。
