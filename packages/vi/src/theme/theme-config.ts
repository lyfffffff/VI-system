...
/** 对外暴露的主题预设列表。 */
export const THEME_PRESETS: IThemePreset<ThemeColorKey>[] =
  THEME_PRESET_SOURCE.map((item, index) => ({
    ...item,
    order: index + 1,
...
/** 主题 key 到预设详情的快速查询表。 */
export const THEME_PRESET_MAP = THEME_PRESETS.reduce<
  Record<ThemeColorKey, IThemePreset<ThemeColorKey>>
>(
  (acc, item) => {
    acc[item.key] = item;
    return acc;
